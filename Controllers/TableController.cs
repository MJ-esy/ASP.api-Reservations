...
namespace ASP_Reservations.Controllers
{
    ...
    public class TableController : ControllerBase
    {
        ...
        public async Task<ActionResult> FindAvailableTables(
        [FromBody] FindAvailableTableDTO dto)
        {
            var tables = await _tableServices.FindAvailableTablesAsync(
                dto.GuestNum,
                dto.StartDateTime
            );

            if (!tables.Any())
                return NotFound(new { message = "No tables available" });
